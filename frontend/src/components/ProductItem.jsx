...
const ProductItem = ({ id, image, name, price }) => {
  ...
  return (
    <Link className='text-gray-700 cursor-pointer h-full' to={id ? `/product/${id}` : '#'}>
      <div className='overflow-hidden aspect-[3/4]  mb-3'>
        <img className='hover:scale-110 transition ease-in-out h-full w-full' src={productImage} alt={name || "Product"} />
      </div>
